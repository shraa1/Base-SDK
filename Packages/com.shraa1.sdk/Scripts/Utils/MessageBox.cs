...
namespace BaseSDK.Utils {
	public class MessageBox : Singleton<MessageBox> {
		...
		public List<GameObject> buttonsPanels = new();
		public Image bg;
		public Text title;
		public Text body;
		public Font font;
		...
		new public static MessageBox Instance {
			get {
				if (instance == null) {
					var allComponentsOfType = FindObjectsByType<MessageBox>(FindObjectsSortMode.None);
				...
				Instance.buttonsPanels.ForEach(x => {
					...
					if (setActive) {
						for (sbyte i = 0; i < buttonsTexts.Length; i++) {
							...
							var txt = btn.transform.Find(TextStr).GetComponent<Text>();
