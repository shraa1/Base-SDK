...
namespace BaseSDK.Helper {
	public class PlayerPrefsManager : MonoBehaviour {
		...
		public static T Get<T>(string key, T defaultValue = default, bool useNewtonsoft = true) {
			...
			var obj = str.Deserialize<PlayerPrefsSaveState>(GameConstants.JsonSerializerSettings);
			var deserialized = obj.Encrypted ? obj.Data.Decrypt<string>().Deserialize<T>() : obj.Data.Deserialize<T>(GameConstants.JsonSerializerSettings);
		...
		public static void Set<T> (string key, T value, bool useNewtonsoft = true, bool encrypted = true) {
			var state = new PlayerPrefsSaveState() { Encrypted = encrypted, Data = encrypted ? value.Serialize(GameConstants.JsonSerializerSettings).Encrypt() : value.Serialize(GameConstants.JsonSerializerSettings) };
			var str = useNewtonsoft ? state.Serialize(GameConstants.JsonSerializerSettings) : JsonUtility.ToJson(value);
